...
function findFuzzyMatch(target: string, words: string[]): { confidence: number } | undefined {
  if (target.length < 6) return undefined; // too short to fuzzy-match safely (false positive risk) — e.g. "Nike" vs "like", "Elsa" vs "else", "Apple" vs "apply" are all 1 edit apart from common English words
  let best = Infinity;
